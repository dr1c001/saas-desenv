  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-wrap items-end gap-3">
      <div className="space-y-1">
        <Label htmlFor="from">{t("reports.periodPicker.fromLabel")}</Label>
        <Input id="from" name="from" type="date" defaultValue={defaultFrom} className="w-36" />
      </div>
      <div className="space-y-1">
        <Label htmlFor="to">{t("reports.periodPicker.toLabel")}</Label>
        <Input id="to" name="to" type="date" defaultValue={defaultTo} className="w-36" />
      </div>
      <Button type="submit" disabled={isPending}>
        {isPending ? t("reports.periodPicker.applying") : t("reports.periodPicker.apply")}
      </Button>
    </form>
  )
}
