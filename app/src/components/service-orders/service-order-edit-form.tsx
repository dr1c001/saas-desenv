...
export function ServiceOrderEditForm({ order, clients }: Props) {
  ...
  const boundAction = updateServiceOrder.bind(null, order.id)
  const [state, formAction, isPending] = useActionState<OrderFormState, FormData>(boundAction, {})
  ...
  const scheduledDefault = order.scheduledAt
