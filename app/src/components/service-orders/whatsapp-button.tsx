  ...
  async function handle() {
    ...
    try {
      ...
      if (data.ok) {
        ...
        setState("error")
        setMsg(data.error ?? t("whatsappButton.sendError"))
        setTimeout(() => setState("idle"), 5000)
      }
    } catch {
      setState("error")
      setMsg(t("whatsappButton.connectionError"))
      setTimeout(() => setState("idle"), 5000)
    }
  }
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {state === "loading" ? t("whatsappButton.sending") : state === "ok" ? t("whatsappButton.sent") : t("whatsappButton.label")}
      </button>
      {state === "error" && msg && (
